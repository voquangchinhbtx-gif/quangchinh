...
# =========================
# LẤY THỜI TIẾT METEO (OPEN-METEO)
# =========================
...
def fetch_meteo_weather(lat: float, lon: float) -> dict:
    ...
    try:
        ...
        st.warning("Không thể lấy dữ liệu thời tiết Meteo (timeout).")
    except requests.exceptions.RequestException as e:
        st.warning(f"Lỗi kết nối Meteo: {e}")
    except (KeyError, ValueError):
        st.warning("Dữ liệu thời tiết Meteo không hợp lệ.")
...
"""
                                res      = model.generate_content(std_prompt, request_options={"timeout": 30})
...
"""
                                res          = model.generate_content(analyze_prompt, request_options={"timeout": 30})
...
"""
                                res         = model.generate_content(prompt_15, request_options={"timeout": 30})
...
# =========================
    ...
    plant_context = ", ".join(plant_names) if plant_names else "Hệ thống tự nhận diện"

    selected_plant = None
    if all_plants:
        selected_plant = st.selectbox(
            "📌 Gắn kết quả vào cây (tùy chọn):",
            [None] + all_plants,
    ...
    if img_file:
        ...
        if st.button("🚀 Phân tích & Kê đơn thuốc", type="primary", key="btn_cam_ai"):

            # Nén ảnh trước khi gửi API (3-5MB → ~200KB)
            buf = io.BytesIO()
            image.convert("RGB").save(buf, format="JPEG", quality=75, optimize=True)
            img_bytes = buf.getvalue()

            # Tính tuổi cây nếu đã chọn
            age_ctx = ""
            if selected_plant:
                try:
                    plant_date = datetime.strptime(selected_plant["date"], "%Y-%m-%d")
...
- Dấu hiệu nhận biết cụ thể từ ảnh.

### 🔬 3. PHÂN TÍCH SÂU TÁC NHÂN
- Nhóm tác nhân (Oomycete / Ascomycete / Xanthomonas / Potyvirus / Bộ cánh vảy...)
- Cơ chế lây lan và điều kiện bùng phát.
- Giai đoạn bệnh hiện tại (Sơ nhiễm / Phát triển / Nặng).

### 🌧️ 4. LIÊN HỆ THỜI TIẾT
Với độ ẩm {w_meteo['hum']}% và nhiệt độ {w_meteo['temp']}°C hiện tại, bệnh có xu hướng lây lan thế nào?
...
"""
    ...
    # Kết quả — NGOÀI if img_file để không bị mất khi chụp ảnh mới
    ...
    if diag:
        ...
        if diag.get("plant"):
            ...
            if st.button(f"💾 Lưu chẩn đoán vào nhật ký '{plant_name}'", key="btn_save_diag"):
