...
with st.sidebar:
    ...
    # ✅ Nút Back
    if "prev_menu" in st.session_state and st.session_state.get("prev_menu") != st.session_state.get("current_menu"):
    ...
    with st.expander("⚙️ Thiết lập & Quản lý danh sách cây", expanded=False):
        ...
        with tab_add:
            ...
            with c2:
                ...
                date_harvest  = st.date_input("🍅 Dự kiến thu hoạch",  value=datetime.now(), key="date_harvest")
                ...
                if name_in.strip() and type_in.strip():
                    ...
                    data    = add_plant(
                        ...
                        extra={
                            ...
                            "date_harvest":   date_harvest.strftime("%Y-%m-%d"),
                        }
                    )
                    save_data(data)
        ...
        with tab_edit:
            ...
            if all_p:
                ...
                with c2:
                    ...
                    harvest_date = p_edit.get("date_harvest", "Chưa có")
                    st.text_input(
                        "🍅 Dự kiến thu hoạch (do AI đề xuất)",
                        value=harvest_date,
                        disabled=True,
        ...
        for p in plants_list:
            with st.container(border=True):
                ...
                with col_care:
                    ...
                    # ✅ NÚT 1: Tạo quy trình chuẩn từ đầu
                    if st.button("🌱 Tạo quy trình chuẩn", key=f"btn_std_{p['id']}"):
                        with st.spinner("AI đang tạo quy trình chuẩn cho cây..."):
                            try:
...
"""
                    ...
                    # Hiển thị quy trình chuẩn nếu có
...
"""
                    ...
                    # Hiển thị phân tích hôm nay nếu có
                    ...
                    # ✅ NÚT 3: Tối ưu 15 ngày tới (giữ nguyên)
...
"""
                    ...
                    if plan_display:
                        with st.expander("📍 XEM QUY TRÌNH ĐÃ LƯU", expanded=False):
